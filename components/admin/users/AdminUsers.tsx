      ...
      <div className="flex flex-col md:flex-row justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            Manage company users, linguists, and clients.
          </p>
        </div>
        <Button onClick={() => setIsAddUserOpen(true)} leftIcon={<UserPlus size={18} />}>
          Add User
        </Button>
      </div>

      <TabsList>
        <TabsTrigger active={activeTab === 'active'} onClick={() => setActiveTab('active')}>
          Active Users
        </TabsTrigger>
        <TabsTrigger active={activeTab === 'pending'} onClick={() => setActiveTab('pending')}>
          Pending Approval
        </TabsTrigger>
        <TabsTrigger active={activeTab === 'removed'} onClick={() => setActiveTab('removed')}>
          Suspended
        </TabsTrigger>
      </TabsList>
