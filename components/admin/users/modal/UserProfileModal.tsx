...
export const UserProfileModal = ({ user, onClose, onUpdate, onNavigate }: UserProfileModalProps) => {
  ...
  const modalContent = (
    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="absolute inset-0 bg-jambo-950/70 backdrop-blur-sm" onClick={onClose}></div>
      
      <div className="bg-white dark:bg-[#13111c] w-full max-w-4xl rounded-2xl shadow-2xl relative z-10 flex flex-col max-h-[90vh] overflow-hidden animate-in zoom-in-95 duration-200 border border-white/10">
        
        {/* Fixed Close Button (Floating) */}
        <button 
            onClick={onClose} 
            className="absolute top-4 right-4 p-2 bg-black/30 hover:bg-black/50 text-white rounded-full transition-colors backdrop-blur-md z-30 border border-white/20"
        >
            <X size={20} />
        </button>
