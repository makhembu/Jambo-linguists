...
import { 
  ...
  Calendar, List, MessageSquare, PenTool
} from 'lucide-react';
...
export const AdminSidebar = ({ activeSection, setActiveSection }: AdminSidebarProps) => {
  ...
  useEffect(() => {
    const updateUnread = () => {
      if (currentUser) setUnreadCount(mockDb.getGlobalUnreadCount(currentUser.id));
    };
    updateUnread();
  ...
  return (
    <aside className="hidden lg:flex flex-col w-72 bg-[#841BA0] dark:bg-jambo-950 text-white shadow-2xl relative z-40 border-r border-white/10 h-screen transition-all duration-300">
      ...
      <div className="h-20 flex items-center px-6 border-b border-white/10 shrink-0 bg-black/10 backdrop-blur-md">
        <div className="flex items-center gap-3 cursor-pointer w-full group" onClick={() => setActiveSection('dashboard')}>
          <div className="h-10 w-10 bg-white rounded-lg flex items-center justify-center p-1 shadow-lg group-hover:scale-105 transition-transform">
            <img src={COMPANY_INFO.logoUrl} alt="Logo" className="h-full w-full object-contain" />
          </div>
          ...
          return (
            <div key={item.id} className="mb-1">
              ...
              >
                ...
                {hasSubItems && <ChevronRight size={16} className={`text-jambo-300 transition-transform duration-300 ${isOpen ? 'rotate-90 text-white' : ''}`} />}
                {isActive && !hasSubItems && <div className="absolute left-0 top-1/2 -translate-y-1/2 w-1 h-8 bg-brand-orange rounded-r-full"></div>}
              </button>
