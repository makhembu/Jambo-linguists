...
import { Clock, Star, AlertTriangle, ArrowRight, ChevronDown, ChevronUp } from 'lucide-react';
...
export const OngoingJobsWidget = ({ onNavigate }: OngoingJobsWidgetProps) => {
  ...
  const getUrgency = (job: Job) => {
      ...
      const diffHrs = (jobTime.getTime() - now.getTime()) / (1000 * 60 * 60);

      if (diffHrs < 0) return { label: 'Overdue', color: 'text-red-500 bg-red-50 dark:bg-red-900/20' };
      if (diffHrs < 24) return { label: 'Due Today', color: 'text-orange-500 bg-orange-50 dark:bg-orange-900/20' };
      return { label: 'Upcoming', color: 'text-blue-500 bg-blue-50 dark:bg-blue-900/20' };
  };

  const getLinguist = (id?: string | null) => {
      if (!id) return null;
      return mockDb.getAllUsers().find(u => u.id === id);
  };
  ...
  return (
    <>
        <div className="bg-white dark:bg-[#13111c] rounded-xl shadow-sm border border-gray-200 dark:border-white/5 overflow-hidden flex flex-col transition-all">
            <div className="p-5 border-b border-gray-100 dark:border-white/5 flex justify-between items-center bg-gray-50/50 dark:bg-white/5 cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
                ...
                <div className="flex items-center gap-2">
                    <button 
                        onClick={(e) => { e.stopPropagation(); onNavigate && onNavigate('jobs', 'Scheduled'); }}
                ...
                <div className="animate-in slide-in-from-top-2 duration-300">
                    ...
                    <div className="md:hidden divide-y divide-gray-100 dark:divide-white/5">
                        {jobs.map(job => {
                            ...
                            return (
                                <div key={job.id} className="p-4 space-y-3 cursor-pointer hover:bg-gray-50 dark:hover:bg-white/5" onClick={() => setSelectedJob(job)}>
                                    <div className="flex justify-between items-start">
                                        <div>
                                            <span className={`text-[10px] font-bold px-2 py-1 rounded-md flex items-center gap-1 w-fit ${urgency.color}`}>
                                    ...
                                    {linguist && (
                                        <div onClick={(e) => handleLinguistClick(e, linguist.id)} className="flex items-center gap-2 p-2 bg-gray-50 dark:bg-white/5 rounded-lg">
                                            <div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-white/10 flex items-center justify-center overflow-hidden border border-white dark:border-white/10 text-xs font-bold text-gray-500">
                                                    {linguist.avatarUrl ? <img src={linguist.avatarUrl} className="w-full h-full object-cover"/> : linguist.firstName[0]}
                                            </div>
                                            <div>
                        ...
                        <table className="w-full text-left text-sm">
                            ...
                            <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                                {jobs.map(job => {
                                    ...
                                    return (
                                        <tr key={job.id} className="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors group cursor-pointer" onClick={() => setSelectedJob(job)}>
                                            <td className="px-5 py-3 whitespace-nowrap">
                                                <span className={`text-[10px] font-bold px-2 py-1 rounded-md flex items-center gap-1 w-fit ${urgency.color}`}>
                                                    {urgency.label === 'Overdue' && <AlertTriangle size={10} />}
                                                    {urgency.label}
                                                </span>
                                                <p className="text-[10px] text-gray-400 mt-1">{job.date} • {job.time}</p>
                                            </td>
                                            <td className="px-5 py-3 max-w-[200px]">
                                                <p className="font-bold text-gray-900 dark:text-white truncate">{job.title}</p>
                                                <p className="text-xs text-gray-500 truncate">{job.category} • {job.location}</p>
                                            </td>
                                            <td className="px-5 py-3">
                                                {linguist ? (
                                                    <div className="flex items-center gap-2" onClick={(e) => handleLinguistClick(e, linguist.id)}>
                                                        <div className="w-8 h-8 rounded-full bg-gray-200 dark:bg-white/10 flex items-center justify-center overflow-hidden border border-white dark:border-gray-800 shrink-0">
                                                            {linguist.avatarUrl ? <img src={linguist.avatarUrl} className="w-full h-full object-cover" alt={`${linguist.firstName}'s avatar`} /> : <span className="text-xs font-bold text-gray-500">{linguist.firstName[0]}</span>}
                                                        </div>
                                                        <div>
                                                            <p className="font-bold text-gray-800 dark:text-gray-200 text-xs hover:text-jambo-600 hover:underline">{linguist.firstName} {linguist.lastName}</p>
                                                            <p className="text-[10px] text-orange-500 font-bold flex items-center gap-0.5">
